...
Convert one currency to another and round, ceil or floor it. Exchange retains the currency code of the currency before the operation.
  ...
  5.34.in(:usd).to(:eur).floor.from                    #=> :usd
...
Access the original currency name after conversion and go back
  ...
  converted2.from                                      #=> :usd
  converted2.revert                                    #=> #<Exchange::Money @value=2.12 @currency=:usd>
...
The options available are
  ...
  :trace (default false)         If set to true, each currency will have a 'from' attribute set with its «predecessor», meaning the currency it was before a conversion or mathematical operation, which is great for debugging when and how a currency was changed.
