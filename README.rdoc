...
The Exchange Gem gives you easy access to currency functions directly on your Numbers. It has been tested against ruby 1.8.7, ruby 1.9.2 and 1.9.3. 
...
but if you have another API you like to use, it becomes as easy as writing one Class and two methods to use it with the Exchange gem.
...
But, same here, if you don't like any of these or want to use your own caching solution, it is as easy as writing one Class and two methods to use it with Exchange
...
All operations with a currency defined as historic will use the historic exchange rate
  3.eur + 345.sek(:at => Time.gm(2011,3,3))       #Will use the March 3 2011 Rate of SEK to EUR to add the currency!
  4.eur + 3.nok.to_sek(:at => Time.gm(2011,3,3))  #Will use the March 3 2011 Rate of SEK to EUR to add the currency!
  
This is also true, because it uses the same historic conversion rate
...
Access the original currency and its value after conversion
...
You can also just instantiate currencies and apply operations
  3.eur.round                                   #=> #<Exchange::Currency @value=3 @currency=:eur>
...
Compare without converting first
  5.eur == 4.34.chf                             #=> true
  50.eur == 4.34.chf                            #=> false
  50.eur.to_sek == 50.eur                       #=> true
  50.eur(:at => '2011-1-1') == 50.sek           #=> false
  
Define a currency object as historic. It will use historic conversion if it gets converted
